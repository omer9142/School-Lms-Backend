...
// ✅ Get all students whose class is unassigned
const getUnassignedStudents = async (req, res) => {
  try {
    const { schoolId } = req.params;

    // Find students that belong to this school and have no class assigned
...
const assignStudentToClass = async (req, res) => {
  try {
    ...
    console.log('Assigning student:', studentId, 'to class:', classId); // Debug log
    ...
    // Check if student exists and is unassigned
    const existingStudent = await Student.findById(studentId);
    if (!existingStudent) {
      return res.status(404).json({ message: "Student not found" });
    }
    ...
    if (existingStudent.sclassName && existingStudent.sclassName.toString() === classId) {
      return res.status(400).json({ message: "Student is already assigned to this class" });
    }

    // Update student's sclassName
    const student = await Student.findByIdAndUpdate(
      studentId,
      { 
        sclassName: classId,
    ...
    console.log('Student updated successfully:', student._id, 'assigned to class:', student.sclassName); // Debug log
...
const removeAllStudentsFromClass = async (req, res) => {
  try {
    ...
    // Update all students in this class to remove their class assignment
    const updateResult = await Student.updateMany(
      { sclassName: classId },
      { 
        $unset: { sclassName: "" } // This removes the sclassName field entirely
      }
    );

    console.log('Students updated:', updateResult);

    // Clear the class students array (if you have one in your schema)
    sclass.students = [];
    await sclass.save();

    res.status(200).json({ 
      message: "All students removed from class successfully",
      studentsUnassigned: updateResult.modifiedCount 
    });
