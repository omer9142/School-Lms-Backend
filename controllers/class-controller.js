...
const removeStudentFromClass = async (req, res) => {
  try {
    ...
    // Make sure the student currently belongs to this class
    const student = await Student.findOne({ _id: studentId, sclassName: classId });
    if (!student) {
      return res.status(404).json({ message: "Student not found in this class" });
    }

    // Remove the link to the class
    student.sclassName = null;      // ✅ unlink the class
    await student.save();
...
module.exports = { sclassCreate, sclassList, getSclassDetail, getSclassStudents, deleteSclass, deleteSclasses, removeStudentFromClass };