...
const getTeachers = async (req, res) => {
    try {
        let teachers = await Teacher.find({ school: req.params.id })
            ...
            .populate("classTeacherOf", "sclassName"); // ✅ ADD THIS
...
const assignClassTeacher = async (req, res) => {
  ...
  try {
    ...
    // Ensure teacher is assigned to the class first
    if (!teacher.teachSclass.some(id => id.toString() === classId)) {
      return res.status(400).json({ message: "Teacher must be assigned to class first" });
    }

    // Add classId if not already there
    if (!teacher.classTeacherOf.some(id => id.toString() === classId)) {
      teacher.classTeacherOf.push(classId);
      await teacher.save();
    }
    ...
    res.json({ message: "Class teacher assigned successfully", teacher: updatedTeacher });
  } catch (err) {
...
const removeClassTeacher = async (req, res) => {
  const { teacherId, classId } = req.body; // now remove specific class

  try {
    const teacher = await Teacher.findById(teacherId);
    if (!teacher) return res.status(404).json({ message: "Teacher not found" });

    // Remove classId from array
    teacher.classTeacherOf = teacher.classTeacherOf.filter(id => id.toString() !== classId);
    await teacher.save();
    ...
    res.json({ message: "Class teacher removed successfully", teacher: updatedTeacher });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
...
const removeTeacherSubject = async (req, res) => {
  const { teacherId, subjectIds } = req.body; // subjectIds = array of subject _id
  ...
  try {
    ...
    teacher.teachSubject = teacher.teachSubject.filter(
      (subId) => !subjectIds.includes(subId.toString())
    );

    await teacher.save();

    // Also unset teacher from subjects
    await Subject.updateMany(
      { _id: { $in: subjectIds } },
      { $unset: { teacher: "" } }
    );

    res.json({ message: "Subjects removed successfully", teacher });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Failed to remove subjects", error: err.message });
  }
};
...
const getTeacherDetail = async (req, res) => {
  try {
    let teacher = await Teacher.findById(req.params.id)
      ...
      .populate("teachSclass", "sclassName")
      .populate("classTeacherOf", "sclassName"); // ✅ ADD THIS LINE
